...
class Zipformer2(torch.nn.Module):
    ...
    def forward(
        ...
        all_hidden_states = [] # stores each layer's output
...
def _test_zipformer_middle_out(causal: bool):
    logging.info(f"Checking zipformer causal={causal}")
    batch_size = 5
    seq_len = 48
    # Just make sure the forward pass runs.

    model = Zipformer2(
        encoder_dim=(64, 64, 86, 64),
        downsampling_factor=(1, 2,4,2),
        num_encoder_layers=(2,2,3,2),
        encoder_unmasked_dim=(48, 48, 64, 48),
        feedforward_dim=(192, 192, 256, 192),
        num_heads=(4, 4, 4, 4),
        causal=causal
    )
    # Just make sure the forward pass runs.
    ...
    x_out, x_out_lens, all_hidden_states = model(
        x, x_lens, return_middle_out=True
    )
...
if __name__ == "__main__":
    ...
    _test_zipformer_middle_out(False)
    _test_zipformer_middle_out(True)
