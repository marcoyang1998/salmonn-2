    ...
    def train(self, mode: bool = True):
        ...
        if mode:
            ...
            if self.merge_weights and self.merged:
                if self.r > 0:
                    lora_delta = T(self.lora_B @ self.lora_A) * self.scaling
                    # Guard: if lora params were left uninitialised (e.g. not in
                    # the checkpoint and materialised as torch.empty by HuggingFace's
                    # low_cpu_mem_usage path), skip the un-merge to avoid corrupting
                    # the loaded weight.
                    if not lora_delta.isnan().any():
                        self.weight.data -= lora_delta
                self.merged = False
        else:
